...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow(
      "Conway's Game of Life", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
  ...
  while (!quit) {
    while (SDL_PollEvent(&e)) {
      if (e.type == SDL_QUIT) {
        ...
        std::cout << "Simulation started.\n";
                 ...
                 e.button.button == SDL_BUTTON_LEFT && !running) {
        mouseDown = true;
    ...
    for (int row = 0; row < rows; ++row) {
      for (int col = 0; col < cols; ++col) {
        if (filled[row][col]) {
          SDL_Rect cell = {col * GRID_SPACING, row * GRID_SPACING, GRID_SPACING,
                           GRID_SPACING};
